...
import com.circularbyte.TrimSound;
import com.circularbyte.Utility;
import com.circularbyte.callrecorder.RecordingFragment.OnListFragmentInteractionListener;
...
import com.circularbyte.receivers.MyLocalBroadcastReceiver;
...
//        }
     ...
     /*   holder.mView.setOnTouchListener(new View.OnTouchListener() {
        ...
        holder.txttrim.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
//                if(null!=mListener)
//                {
//                    mListener.onTrim(holder.mItem);
//                }

                float time = holder.mItem.getPhoneCall().getEndTime().getTimeInMillis() - holder.mItem.getPhoneCall().getStartTime().getTimeInMillis();
              String duration = String.format("%.2f", ((time / 1000) / 60));

              float val = ((time / 1000) / 60)*100;

                int times = Math.round(val);

              String path = holder.mItem.getPhoneCall().getPathToRecording();
              String name = holder.mItem.getName();

                Toast.makeText(context, "Times "+times, Toast.LENGTH_SHORT).show();
               //public AudioModel(String name, String path, String duration, String size) {
                Utility.editsound = new AudioModel(name.toString(),path.toString(),times+"","");
                Toast.makeText(context, "Path: "+path.toString(), Toast.LENGTH_SHORT).show();

                context.startActivity(new Intent(context, TrimSound.class));
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View view) {
            ...
            txttrim = (TextView) view.findViewById(R.id.txttrim);
